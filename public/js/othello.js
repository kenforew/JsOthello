...
var canvas,ctx,turn=1,count=0,yourcolor=0;
stonelist=[];
...
function skipJudge(){
    ...
    for(var i=0;i<8;i++){
        for(var j=0;j<8;j++){
            if(stonelist[i][j]==0){
                result=result||reverseJudge(i,j);
            }
        }
    }
    return result;
...
function reverseJudge(x,y){
    var result=false;
    for(var i=-1;i<2;i++){
        for(var j=-1;j<2;j++){
            if(!(i==0&&j==0)){
                result=result||reverseJudge2(x,y,i,j);
            }
        }
    }
    return result;
...
function reverseJudge2(x,y,dx,dy){
    if(x+dx>=0&&x+dx<=7
    &&y+dy>=0&&y+dy<=7){
        if(stonelist[x+dx][y+dy]==-turn){
            var i=2;
...
function gameDisplay(){
    ...
    ctx.fillRect(canvas.width/6,canvas.height/6,canvas.width*4/6,canvas.height*4/6);
            ...
            if(stonelist[i][j]!=0){
                ...
                if(reverseJudge(i,j)){
                    ctx.fillStyle="rgba(144,144,22,"
                        +(Math.sin(count*Math.PI/60)*0.4+0.5)+
                    ")";
                    ctx.beginPath();
                    ctx.arc(canvas.width/6+canvas.width*2/3*(i+0.5)/8,
                    canvas.height/6+canvas.height*2/3*(j+0.5)/8,
...
canvas.addEventListener("click",e=>{
    ...
    if(turn==yourcolor){
        ...
        if(reverseJudge(cx,cy)&&stonelist[cx][cy]==0){
            //reverse(cx,cy);
            //turnChange();
...
socket.on("turnInitialize",(message)=>{
    yourcolor=message;
    console.log("my color : ",yourcolor);
    setLabel();
});
...
socket.on("you_put_stone",(message)=>{
    ...
    console.log("skipjudge : "+skipJudge());
